function(input,output){
    ...
    #tren data kependudukan per kabupaten 2b
    ...
    #Tab 3
    #A
    output$cor_kab <- renderPlot({
        ggc <- ggcorr(for_choose, method = c("pairwise","pearson"),
                      hjust = .85,
                      size = 0,
                      layout.exp=4,
                      label = T)
        
        dat <- data.frame(x = seq(for_choose), y = seq(for_choose),
                          lbs = gsub("_", " ", names(for_choose) ))
        ggc + geom_text(data=dat, aes(x, y, label=lbs), nudge_x = 4, hjust=1)})
    #Tab 3B
