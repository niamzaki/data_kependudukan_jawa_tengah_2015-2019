...
sidebar <- dashboardSidebar(
    sidebarMenu(
        ...
        menuSubItem(text = "Pranala",
                    ...
                    href = "https://jateng.bps.go.id/subject/157/produk-domestik-regional-bruto--kabupaten-kota-.html#subjekViewTab3"))
    ) )
body <- dashboardBody(
    tabItems(
        #Tab 1
        ...
        tabItem(tabName = "kor",
                ...
                #B
                fluidPage(box(width = 9,
                              title = tags$b("Korelasi Antar Parameter Per Kabupaten/Kota di Provinsi Jawa Tengah"),
                              plotlyOutput("cor_kab")
                ),
                box(width = 3,
                    background = "blue",
                    height = 120,
                    selectInput(inputId = "sumbu_x_kab",
                                label = h4(tags$b("Pilih Parameter Sumbu X:")),
                                choices = str_replace_all(colnames(for_choose),"_"," "))),
                box(width = 3,
                    background = "blue",
                    height = 120,
                    selectInput(inputId = "sumbu_y_kab",
                                label = h4(tags$b("Pilih Parameter Sumbu Y:")),
                                choices = str_replace_all(colnames(for_choose),"_"," "))),
                box(width = 3,
                    background = "blue",
                    height = 120,
                    selectInput(inputId = "nama_kab",
                                label = h4(tags$b("Pilih Kabupaten/Kota:")),
                                choices = unique(data_jateng$kabupaten))),
                valueBoxOutput(width = 3,
                               "korelasi_kab")
                )
                ),
